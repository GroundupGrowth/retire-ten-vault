...
const CALL_CTA_DELAY_MS = 5 * 60 * 1000;
...
const Hero = ({ ctaMode = "buy" }: Props) => {
  const [callUnlocked, setCallUnlocked] = useState(ctaMode !== "call");

  useEffect(() => {
    if (ctaMode !== "call") return;
    const timer = setTimeout(() => setCallUnlocked(true), CALL_CTA_DELAY_MS);
    return () => clearTimeout(timer);
  }, [ctaMode]);

  const showCallCta = ctaMode === "call" && callUnlocked;
  const showBookButtons = ctaMode !== "call" || callUnlocked || true; // always show book CTAs
      ...
      <div className="container-page relative">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-sm text-ink-muted">
            {ctaMode === "call" && !callUnlocked
              ? "Grab the eBook or order a signed copy. Booking a call with Barry unlocks once you've seen the masterclass."
        ...
        <div className="text-center">
          <div className="flex flex-wrap items-center justify-center gap-3 mb-4">
            ...
            {ctaMode !== "call" && <MainCTA mode={ctaMode} />}
            <SignedCopyCTA />
          </div>
          <p className="micro-trust max-w-xl mx-auto">
            {ctaMode === "call"
              ? callUnlocked
                ...
                : "Instant PDF download for $19.97 or a signed physical copy for $33.95."
